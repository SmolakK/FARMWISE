...
if dev_test_enabled and __name__ == "__main__":
    ...
    print("\n===== GW LEVEL DATA (in piezometers) IN FRANCE VIA HubEau ======")
...
async def fetch_data(api, pt_id, he_period_bounds, data_requested_varnames, verbose_level):
    ...
    try:
        ...
        # (Note: Here argument names are specific to the hubeaupyutils lib functions, and thus differ from the online HubEau API parameters.)
        df = await asyncio.to_thread(
            api.get_data,
            code_station=pt_id,
            start_date=he_period_bounds[0],
            end_date=he_period_bounds[1],
            only_valid_data=True, # Remark: This will also filter out "dynamic" water levels... but that is okay for the purposes of the study.
            add_real_time=False,
        ...
        # Remark: Those column names and contents result from special formatting of the output by hubeaupyutils lib's "piezometry" wrapper (e.g., 'indice').
        if not df.empty:
...
async def read_data(spatial_range, time_range, data_range, level, nmax_pts=None, verbose_level=0):
    ...
    data_requested_keys_set = data_asked_raw_set & set(['gwlevel']) # Note that 'gwdepth' is no longer allowed!
...
if dev_test_enabled and __name__ == "__main__":
    ...
    E = -2.0
    W = -5.0
    LEVEL = 15 # Cells levels range from 0 to 30. The smallest cells at level 30 are called leaf cells
    ...
    data_range = ['gwlevel']  # ['phosphorus', 'pesticides','pfas'][2] #None #'pesticides'
    ...
    print("\nTEST: calling function read_data of hubeau_piezo_read...")

    whatweget = asyncio.run(read_data(spatial_range, time_range, data_range, LEVEL, nmax_pts=2, verbose_level=2))

...
def read_and_modify_csv(file_path, custom_headers, new_data, skip_until=81, num_rows=3):
...
def process_all_csv(data_directory, output_directory, custom_headers, new_data, skip_until=81, num_rows=3):
    ...
    output_path = os.path.join(output_directory, 'others.csv')
...
new_data = ['radioactivity [Bq*kg-1]', 'soil_electrical_conductivity [mS*m-1]', 'soil_salinity [mg KCl*100g-1]']

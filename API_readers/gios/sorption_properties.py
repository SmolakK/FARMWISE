...
def read_and_modify_csv(file_path, custom_headers, new_data, skip_until=14, num_rows=10):
...
def process_all_csv(data_directory, output_directory, custom_headers, new_data, skip_until=14, num_rows=10):
    ...
    output_path = os.path.join(output_directory, 'sorption_properties.csv')
...
new_data = ['Hh [cmol(+)*kg-1]', 'Hw [cmol(+)*kg-1]', 'Al [cmol(+)*kg-1]', 'Ca2+ [cmol(+)*kg-1]', 'Mg2+ [cmol(+)*kg-1]',

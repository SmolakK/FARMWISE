...
def read_and_modify_csv(file_path, custom_headers, new_data, skip_until=1, num_rows=6):
...
def process_all_csv(data_directory, output_directory, custom_headers, new_data, skip_until=1, num_rows=6):
    ...
    output_path = os.path.join(output_directory, 'grain_size_distribution.csv')
...
new_data = ['1.0-0.1 mm [%]', '0.1-0.02 mm [%]', '< 0.02 mm [%]', '2.0-0.05 mm [%]', '0.05-0.002 mm [%]',
            '< 0.002 mm [%]']

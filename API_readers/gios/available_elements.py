...
def read_and_modify_csv(file_path, custom_headers, new_data, skip_until=24, num_rows=6):
...
def process_all_csv(data_directory, output_directory, custom_headers, new_data, skip_until=24, num_rows=6):
    ...
    output_path = os.path.join(output_directory, 'available_elements.csv')
...
new_data = ['available phosphorus [mg P2O5* 100g-1]', 'available potassium [mg K2O*100g-1]',
            'available magnesium [mg Mg*100g-1]', 'available sulphur [mg S-SO4*100g-1]',
            'available ammonium nitrogen[NNH4 mg*kg-1]', 'available nitrate nitrogen [NNO3 mg*kg-1]']

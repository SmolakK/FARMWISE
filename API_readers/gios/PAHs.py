...
def read_and_modify_csv(file_path, custom_headers, new_data, skip_until=54, num_rows=16):
...
def process_all_csv(data_directory, output_directory, custom_headers, new_data, skip_until=54, num_rows=16):
    ...
    output_path = os.path.join(output_directory, 'PAHs.csv')
...
new_data = ['PAH_sum_13 [µg*kg-1]', 'naphthalene [µg*kg-1]', 'phenanthrene [µg*kg-1', 'anthracene [µg*kg-1]',
            'fluoranthene [µg*kg-1]', 'chrysene [µg*kg-1]', 'benzo(a)anthracene [µg*kg-1]', 'benzo(a)pyrene [µg*kg-1]',
            'benzo(a)fluoranthene [µg*kg-1]', 'benzo(ghi)perylene [µg*kg-1]', 'fluorene [µg*kg-1]', 'pyrene [µg*kg-1]',
            'benzo(b)fluoranthene [µg*kg-1]', 'benzo(k)fluoranthene [µg*kg-1]', 'dibenzo(a.h)anthracene [µg*kg-1]',
            'indeno(1.2.3-cd)pyrene [µg*kg-1]']

...
def _s2cell_id_to_coordinate(s2cell_id):
    ...
    """
    cell = s2sphere.CellId(s2cell_id)
    cell_center = cell.to_lat_lng()
...
def s2cells_to_coordinates(pivoted_table):
    ...
    transposed = pivoted_table.T

    # Extract S2Cell IDs from the index
    s2cells = transposed.index.get_level_values(1)
    ...
    transposed[['lat', 'lon']] = s2_coordinates

    # Reset the index to convert the transposed DataFrame back to its original format
    transposed = transposed.reset_index()

    # Drop the original S2CELL column from the DataFrame
    transposed = transposed.drop('S2CELL', axis=1)
